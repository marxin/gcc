...
static INLINE void CheckForInvalidPointerPair(void *p1, void *p2) {
  ...
  if (offset <= 2048) {
    if (__asan_region_is_poisoned(left, offset) == 0)
      ...
      REPORT_INVALID_POINTER_PAIR;
  }

  uptr shadow_offset1, shadow_offset2;

  {
    bool is_left_on_stack;
    {
  ...
  bool is_right_on_stack;
  {
    ThreadRegistryLock l(&asanThreadRegistry());
    is_right_on_stack = GetStackVariableBeginning(right - 1, &shadow_offset2);
  }

  if (is_right_on_stack)
    REPORT_INVALID_POINTER_PAIR;
