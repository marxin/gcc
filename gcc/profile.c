                          ...
                          unsigned lineno_checksum)
{
  ...
  for (i = 0; i < values.length (); i++)
	...
	{
	  struct loop *loop = hist->hvalue.loop;
	  loop->iteration_histogram = ggc_alloc<loop_histogram> ();
	  loop->iteration_histogram->zero_executions = hist->hvalue.counters[0];
	  loop->iteration_histogram->min_niter = hist->hvalue.counters[1];
	  loop->iteration_histogram->max_niter = hist->hvalue.counters[2];
	}
    }
