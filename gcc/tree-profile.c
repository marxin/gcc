...
void
gimple_gen_loop_profiler (histogram_value value)
{
  edge e;
  edge_iterator ei;
  struct loop *loop= value->hvalue.loop;
  basic_block header = loop->header;
  gimple_stmt_iterator gsi = gsi_start_nondebug_after_labels_bb (header);
  tree ref_ptr = tree_coverage_counter_addr (GCOV_COUNTER_LOOP, 0);
  ...
  for (unsigned i = 0; i < loop_exits.length (); i++)
    {
      edge e = loop_exits[i];
      // FIXME: do something with abnormal edges
