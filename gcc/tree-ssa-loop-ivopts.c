...
struct comp_cost
{
  comp_cost (): cost (0), cost_scalled (0.0f), complexity (0), scratch (0),
    frequency (0.0f) {}
  ...
  comp_cost operator+= (int c)
    {
      this->cost += c;
      this->cost_scalled += frequency > 0.0f ? frequency * c : c;
  ...
  friend bool operator== (comp_cost cost1, comp_cost cost2)
  {
    return cost1.cost == cost2.cost && cost1.complexity == cost2.complexity;
  }
  ...
  float get_cost_scalled ()
    {
      return cost_scalled;
    }

  void calculate_scalled_cost (float f)
    {
      // TODO: catch that!
      if (f > 1.0f)
	f = 1.0f;

      cost_scalled = f > 0.0f ? cost * f : cost;
      frequency = f;
    }
  ...
  void hack_cost ()
	...
	return;
      ...
      cost = (int)round(cost_scalled);
    }

private:  
  int cost;		/* The runtime cost.  */
  float cost_scalled;   /* The runtime cost.  */
                         ...
                         int *inv_expr_id)
{
  ...
  float frequency_ratio = 0.0f;
  if (data->current_loop->header->frequency && at->bb->frequency)
    frequency_ratio
      = 1.0f * at->bb->frequency / data->current_loop->header->frequency;
...
ret:
  cost.calculate_scalled_cost (frequency_ratio);
  return cost;
}
				 ...
				 struct iv_use *use, struct iv_cand *cand)
{
  ...
  if (!cand->iv)
    {
      set_use_iv_cost (data, use, cand, comp_cost::get_infinite (), NULL, NULL_TREE,
		       ERROR_MARK, -1);
      return false;
    }
  ...
  float frequency_ratio = 0.0f;
  if (data->current_loop->header->frequency && use->stmt->bb->frequency)
    frequency_ratio
      = 1.0f * use->stmt->bb->frequency / data->current_loop->header->frequency;
...
iv_ca_dump (struct ivopts_data *data, FILE *file, struct iv_ca *ivs)
{
  ...
  fprintf (file, "  cost: %d (complexity %d, scalled: %2.2f)\n", cost.get_cost (), cost.complexity, cost.get_cost_scalled ());
   ...
   for (i = 0; i < ivs->upto; i++)
    {
      ...
      if (cp)
        fprintf (file, "   use:%d --> iv_cand:%d, cost=(%d,%d,%2.2f,%2.2f)\n",
                 use->id, cp->cand->id, cp->cost.get_cost (), cp->cost.complexity,
		 cp->cost.frequency, cp->cost.get_cost_scalled ());
      else
        fprintf (file, "   use:%d --> ??\n", use->id);
    }
