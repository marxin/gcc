...
ipcp_cloning_candidate_p (struct cgraph_node *node)
{
  ...
  if (inline_summaries->get_or_insert (node)->self_size < stats.n_calls)
    {
      if (dump_file)
...
estimate_local_effects (struct cgraph_node *node)
{
  ...
  vec<ipa_agg_jump_function_p> known_aggs_ptrs;
  bool always_const;
  int base_time = inline_summaries->get_or_insert (node)->time;
  int removable_params_cost;
...
ipcp_propagate_stage (struct ipa_topo_info *topo)
{
  ...
  FOR_EACH_DEFINED_FUNCTION (node)
  {
    ...
    if (node->definition && !node->alias)
      overall_size += inline_summaries->get_or_insert (node)->self_size;
