...
process_hsa_functions (void)
{
  ...
  FOR_EACH_DEFINED_FUNCTION (node)
    {
      hsa_function_summary *s = hsa_summaries->get2 (node);
  ...
  FOR_EACH_DEFINED_FUNCTION (node)
    {
      ...
      while (e)
	{
	  hsa_function_summary *src = hsa_summaries->get2 (node);
	  if (src != NULL
	      && src->m_gpu_implementation_p)
	    {
	      hsa_function_summary *dst = hsa_summaries->get2 (e->callee);
  ...
  for (i = 0; i < count; i++)
    {
      ...
      hsa_function_summary *s = hsa_summaries->get_or_insert (node);
