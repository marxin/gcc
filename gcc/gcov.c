      ...
      for (unsigned i = 0; i < block->locations.size (); i++)
	{
	  ...
	  for (unsigned j = 0; j < lines.size (); j++)
	      ...
	      if (!block->exceptional)
		{
		  ...
		  if (block->count == 0 && block->is_call_return)
  ...
  if (exists)
      ...
      if (count > 0)
	{
	  ...
	  if (has_unexecuted_block)
		...
		{
		  pad_count_string (s);
		  s = SGR_SEQ (COLOR_BG_MAGENTA COLOR_SEPARATOR COLOR_FG_WHITE) + s + SGR_RESET;
		}
	      else
		s += "*";
	    }
	  ...
	  if (flag_use_colors)
	    {
	      ...
	      pad_count_string (s);
	      if (unexceptional)
		s.insert (0, SGR_SEQ (COLOR_BG_RED COLOR_SEPARATOR COLOR_FG_WHITE));
	      else
		s.insert (0, SGR_SEQ (COLOR_BG_CYAN COLOR_SEPARATOR COLOR_FG_WHITE));
	      s += SGR_RESET;
	    }
	  else
	    {
