...
{
  hsa_function_summary *gpu_summary = get_or_insert (gpu);
  hsa_function_summary *host_summary = get_or_insert (host);
...
hsa_register_kernel (cgraph_node *host)
{
  ...
  hsa_function_summary *s = hsa_summaries->get_or_insert (host);
  s->m_kind = HSA_KERNEL;
}
