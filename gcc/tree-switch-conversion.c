...
switch_conversion::collect (gswitch *swtch)
{
  ...
  index_expr = gimple_switch_index (swtch);
  switch_bb = gimple_bb (swtch);
  default_bb
    = label_to_block (CASE_LABEL (gimple_switch_default_label (swtch)));
  e_default = find_edge (switch_bb, default_bb);
  default_prob = e_default->probability;
  default_count = e_default->count;
  ...
  tree last = CASE_HIGH (min_case) ? CASE_HIGH (min_case) : range_min;
  ...
  if (contiguous_range)
    {
      ...
      e_first = find_edge (switch_bb, first);
    }
  else
    {
      first = default_bb;
  ...
  if (final_bb)
	...
	if (single_pred_p (e->dest)
	    ...
	    && single_succ (e->dest) == final_bb)
	  continue;

	if (e == e_default && contiguous_range)
	  {
	    default_case_nonstandard = true;
	    continue;
	  }
  ...
  range_size
    = int_const_binop (MINUS_EXPR, range_max, range_min);
...
switch_conversion::check_range ()
{
  ...
  if (!tree_fits_uhwi_p (range_size))
    {
      reason = "index range way too large or otherwise unusable";
      return false;
    }

  if (tree_to_uhwi (range_size)
      > ((unsigned) count * SWITCH_CONVERSION_BRANCH_RATIO))
    {
...
{
  edge e, e_default = find_edge (switch_bb, default_bb);
  edge_iterator ei;
  ...
  FOR_EACH_EDGE (e, ei, switch_bb->succs)
	...
	{
	  if (contiguous_range && e == e_default)
	    {
	      default_case_nonstandard = true;
	      continue;
	    }
...
switch_conversion::check_final_bb ()
{
  ...
  phi_count = 0;
  for (gsi = gsi_start_phis (final_bb); !gsi_end_p (gsi); gsi_next (&gsi))
    {
      gphi *phi = gsi.phi ();
      unsigned int i;
      ...
      for (i = 0; i < gimple_phi_num_args (phi); i++)
	{
	  ...
	  if (bb == switch_bb
	      ...
	      if (reason)
		{
		  ...
		  if (bb == switch_bb)
		    bb = final_bb;
		  if (!contiguous_range || bb != default_bb)
		    {
		      reason = reason;
		      return false;
		    }
...
switch_conversion::create_temp_arrays ()
{
  ...
  default_values = XCNEWVEC (tree, phi_count * 3);
  ...
  constructors = XCNEWVEC (vec_constructor_elt_gc, phi_count);
  target_inbound_names = default_values + phi_count;
  target_outbound_names = target_inbound_names + phi_count;
  for (i = 0; i < phi_count; i++)
    vec_alloc (constructors[i], tree_to_uhwi (range_size) + 1);
}

void
switch_conversion::free_temp_arrays ()
{
  XDELETEVEC (constructors);
  XDELETEVEC (default_values);
}
...
switch_conversion::build_constructors ()
{
  ...
  for (i = 1; i < branch_num; i++)
	...
	{
	  int k;
	  gcc_assert (!contiguous_range);
	  for (k = 0; k < phi_count; k++)
	    {
	      constructor_elt elt;

	      elt.index = int_const_binop (MINUS_EXPR, pos, range_min);
	      elt.value
		= unshare_expr_without_location (default_values[k]);
	      constructors[k]->quick_push (elt);
	    }
	...
	high = CASE_LOW (cs);
      for (gsi = gsi_start_phis (final_bb);
...
tree
switch_conversion::constructor_contains_same_values_p (vec<constructor_elt, va_gc> *vec)
{
  unsigned int i;
  tree prev = NULL_TREE;
  constructor_elt *elt;
...
{
  unsigned int i, len = vec_safe_length (constructors[num]);
  constructor_elt *elt;
  int sign = 0;
  tree smaller_type;
  ...
  FOR_EACH_VEC_SAFE_ELT (constructors[num], i, elt)
    {
      wide_int cst;
				    ...
				    gphi *phi, tree tidx)
{
  ...
  cst = constructor_contains_same_values_p (constructors[num]);
  if (cst)
  ...
  else
    {
      ...
      default_type = TREE_TYPE (default_values[num]);
...
switch_conversion::build_arrays ()
{
  ...
  sub = fold_build2_loc (loc, MINUS_EXPR, utype,
			 fold_convert_loc (loc, utype, index_expr),
			 fold_convert_loc (loc, utype, range_min));
  ...
  for (gpi = gsi_start_phis (final_bb), i = 0;
      ...
      else
	{
	  ...
	  gcc_assert (e && e->dest == final_bb);
	  target_vop = PHI_ARG_DEF_FROM_EDGE (phi, e);
	}
    }
}
...
switch_conversion::gen_def_assigns (gimple_stmt_iterator *gsi)
{
  ...
  for (i = 0; i < phi_count; i++)
    {
      tree name = copy_ssa_name (target_inbound_names[i]);
      target_outbound_names[i] = name;
      assign = gimple_build_assign (name, default_values[i]);
...
switch_conversion::fix_phi_nodes (edge e1f, edge e2f, basic_block bbf)
{
  ...
  for (gsi = gsi_start_phis (bbf), i = 0;
       !gsi_end_p (gsi); gsi_next (&gsi))
    {
      ...
      if (virtual_operand_p (gimple_phi_result (phi)))
	inbound = outbound = target_vop;
      else
	{
	  inbound = target_inbound_names[i];
	  outbound = target_outbound_names[i++];
	}
...
switch_conversion::gen_inbound_check ()
{
  ...
  tidx = gimple_assign_lhs (arr_ref_first);
  utype = TREE_TYPE (tidx);

  /* (end of) block 0 */
  gsi = gsi_for_stmt (arr_ref_first);
  gsi_next (&gsi);

  bound = fold_convert_loc (loc, utype, range_size);
  ...
  e01->probability = default_prob.invert ();
  ...
  prune_bbs (bbd, final_bb,
	     default_case_nonstandard ? default_bb : NULL);
...
switch_conversion::expand (gswitch *swtch)
{
  ...
  gcc_checking_assert (TREE_TYPE (index_expr) != error_mark_node);

  /* A switch on a constant should have been optimized in tree-cfg-cleanup.  */
  gcc_checking_assert (! TREE_CONSTANT (index_expr));
  ...
  create_temp_arrays ();
  gather_default_values (default_case_nonstandard
...
void
switch_decision_tree::compute_cases_per_edge (gswitch *stmt)
{
...
bool
switch_decision_tree::analyze_switch_statement (gswitch *swtch)
{
...
bool
switch_decision_tree::try_switch_expansion (gswitch *stmt,
					    ...
					    const vec<basic_block> &case_bbs)
{
  ...
  emit (bb, stmt, index_expr, index_type, case_list, default_bb, default_prob);
...
void
switch_decision_tree::record_phi_operand_mapping (const vec<basic_block> bbs,
...
void
switch_decision_tree::fix_phi_operands_for_edges (vec<basic_block> case_bbs,
						  hash_map<tree, tree> *phi_mapping)
{
  gphi_iterator gsi;
...
void
switch_decision_tree::emit (basic_block bb, gswitch *s, tree index_expr,
			    tree index_type, case_tree_node *case_list,
			    basic_block default_bb,
  ...
  if (np)
      ...
      if (i > 2)
	{
	  ...
	  *npp = 0;
	  ...
	  np->c->m_subtree_prob = np->c->m_prob;
	  np->c->m_subtree_prob += np->left->c->m_subtree_prob;
	  np->c->m_subtree_prob += np->right->c->m_subtree_prob;
	}
      else
	{
	  ...
	  for (; np->right; np = np->right)
	    {
	      np->right->parent = np;
	      (*head)->c->m_subtree_prob += np->right->c->m_subtree_prob;
	    }
	}
    }
}
				       ...
				       int indent_step, int indent_level)
{
  ...
  root->c->dump (f);
  root->c->m_prob.dump (f);
  root->c->m_subtree_prob.dump (f);
  fputs ("\n", f);
...
pass_lower_switch<O0>::execute (function *fun)
{
  ...
  for (unsigned i = 0; i < switch_statements.length (); i++)
      ...
      if (dump_file)
	{
      ...
      if (swtch)
	expanded |= switch_decision_tree::analyze_switch_statement (swtch);
    }
				       ...
				       tree index_type)
{
  ...
  profile_probability probability
    = node->right ? node->right->c->m_subtree_prob : profile_probability::never ();
  probability = ((probability + default_prob.apply_scale (1, 2))
		 / (node->c->m_subtree_prob + default_prob));
  bb = emit_cmp_and_jump_insns (bb, index, node->c->get_high (), GT_EXPR,
				test_bb, probability);
  default_prob = default_prob.apply_scale (1, 2);

  /* Value belongs to this node or to the left-hand subtree.  */
  probability = node->c->m_prob / (node->c->m_subtree_prob + default_prob);
