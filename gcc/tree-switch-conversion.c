...
expand_degenerated_switch (gswitch *swtch)
{
  ...
  if (high)
    ...
    cond = gimple_build_cond (EQ_EXPR, index, fold_convert (TREE_TYPE (index),
							    low),
			      NULL_TREE, NULL_TREE);
...
expand_switch_conv (gswitch *swtch)
{
  ...
  /* We should not generate switch statements with only a default label.  */
  gcc_assert (gimple_switch_num_labels (swtch) > 1);

  /* If this switch is now a degenerate case with only a default label and
     a single case, convert it to condition.  */
...
{
  /* Constructor.  */
  simple_cluster(tree low, tree high, basic_block case_bb):
  ...
  debug ()
  {
    fprintf (stderr, "%d", tree_to_shwi (get_low ()));
    if (get_low () != get_high ())
      fprintf (stderr, "-%d", tree_to_shwi (get_high ()));
    fprintf (stderr, " ");
  }
  ...
  dump (FILE *f)
  {
    fprintf (f, "%d", tree_to_shwi (get_low ()));
    if (get_low () != get_high ())
      fprintf (f, "-%d", tree_to_shwi (get_high ()));
    fprintf (f, " ");
  }
...
struct group_cluster: public cluster
{
  ...
  debug ()
  {
    fprintf (stderr, "%s:%d-%d ", get_type () == JUMP_TABLE ? "JT" : "BT",
	     tree_to_shwi (get_low ()), tree_to_shwi (get_high ()));
  }

  virtual void
  dump (FILE *f)
  {
    fprintf (f, "%s:%d-%d ", get_type () == JUMP_TABLE ? "JT" : "BT",
	     tree_to_shwi (get_low ()), tree_to_shwi (get_high ()));
  }
...
jump_table_cluster::find_jump_tables (vec<cluster *> &clusters)
{
  ...
  HOST_WIDE_INT min_jump_table_size
  = PARAM_VALUE (PARAM_SWITCH_MIN_JUMP_TABLE_SIZE);
  ...
  for (unsigned i = 1; i <= l; i++)
      ...
      for (unsigned j = 0; j < i; j++)
	{
	  ...
	  if ((min[j].m_count + 1 < min[i].m_count ||
	    ...
	    fprintf (stderr, "%d = %d, start: %d, non_jt_cases: %d\n", i, min[i].m_count,
		     min[i].m_start, min[i].m_non_jt_cases);
	    }
	}
    }
...
jump_table_cluster::can_be_handled (const vec<cluster *> &clusters,
				    unsigned start, unsigned end)
{
  unsigned HOST_WIDE_INT density_threshold
    = PARAM_VALUE (PARAM_SWITCH_MIN_JUMP_TABLE_DENSITY);
  HOST_WIDE_INT max_jump_table_size
    = PARAM_VALUE (PARAM_SWITCH_MAX_JUMP_TABLE_SIZE);

  HOST_WIDE_INT range = get_range (clusters[start]->get_low (),
				   clusters[end]->get_high ());

  if (max_jump_table_size != 0 && range > max_jump_table_size)
    return false;
  ...
  return 100 * value_count >= density_threshold * range;
}
				   ...
				   unsigned start, unsigned end)
{
  ...
  return end - start + 1 >= min_jump_table_size;
}
...
analyze_switch_statement (gswitch *swtch)
{
  ...
  for (unsigned i = 1; i < l; i++)
	...
	high = low;
      ...
      clusters.quick_push (new simple_cluster (low, high, case_bb));
    }
  ...
  bool expanded = try_switch_expansion (swtch, output2);
...
static case_node *
add_case_node (case_node *head, cluster *c,
...
try_switch_expansion (gswitch *stmt, vec<cluster *> &clusters)
{
  ...
  for (int i = clusters.length () - 1; i >= 0; i--)
    case_list
      = add_case_node (case_list, clusters[i], case_node_pool);


  balance_case_nodes (&case_list, NULL);
  fprintf (stderr, ";; Expanding GIMPLE switch as decision tree:\n");
  dump_case_nodes (stderr, case_list, 4, 0);
  ...
  if (node_is_bounded (node, index_type))
  ...
  else if (tree_int_cst_equal (node->c->get_low (), node->c->get_high ()))
    {
	 ...
	 this node and then check our children, if any.  */
      bb = do_jump_if_equal (bb, index, node->c->get_low (), node->c->m_case_bb, probability,
			     phi_mapping);
      ...
      if (node->right != 0 && node->left != 0)
	{
	  ...
	  else if (tree_int_cst_equal (node->right->c->get_low (), node->right->c->get_high ())
