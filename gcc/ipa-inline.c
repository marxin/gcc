		   ...
		   bool disregard_limits = false, bool early = false)
{
  ...
  /* Don't inline a function with a higher optimization level than the
  ...
  else if (caller_tree != callee_tree)
    {
      ...
      inline_summary *caller_info = inline_summaries->get_or_insert (caller);
...
{
  sreal time
    = compute_uninlined_call_time (inline_summaries->get_or_insert (e->callee),
				   e);
  ...
  */
	...
	*/
		      ...
		      || DECL_DECLARED_INLINE_P (caller->decl)))))
	{
	  ...
	  caller_info = inline_summaries->get_or_insert (caller);
      ...
      denominator
	*= inline_summaries->get_or_insert (caller)->self_size + growth;
		    ...
		    vec<cgraph_edge *> *new_edges)
{
  ...
  if (dump_file)
    fprintf (dump_file,
	     ...
	     inline_summaries->get2 (master_clone)->time,
	     inline_summaries->get2 (node)->time);
  ...
  while (!edge_heap.empty ())
      ...
      if (dump_file)
	{
	  fprintf (dump_file,
		   ...
		   "net change of %+i.\n",
		   edge->caller->name (),
		   inline_summaries->get2 (edge->caller)->time,
		   inline_summaries->get2 (edge->caller)->size,
  ...
  while (node->callers && !node->global.inlined_to)
      ...
      if (dump_file)
	{
	  fprintf (dump_file,
		   "\nInlining %s size %i.\n",
		   node->name (),
		   inline_summaries->get2 (node)->size);
	  fprintf (dump_file,
		   " Called once from %s %i insns.\n",
		   node->callers->caller->name (),
		   inline_summaries->get2 (node->callers->caller)->size);
	}
	...
	fprintf (dump_file,
		 ...
		 caller->name (),
		 inline_summaries->get2 (caller)->size);
      if (!(*num_calls)--)
	{
	  if (dump_file)
