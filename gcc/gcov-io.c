...
GCOV_LINKAGE void
gcov_write_histogram (const struct gcov_histogram *histogram)
{
  ...
  for (h_ix = 0; h_ix < GCOV_HISTOGRAM_SIZE; h_ix++)
    if (histogram->histogram[h_ix].num_counters)
      {
  ...
  for (h_ix = 0; h_ix < GCOV_HISTOGRAM_SIZE; h_ix++)
    {
      if (!histogram->histogram[h_ix].num_counters)
       continue;
      gcov_write_unsigned (histogram->histogram[h_ix].num_counters);
      gcov_write_counter (histogram->histogram[h_ix].min_value);
      gcov_write_counter (histogram->histogram[h_ix].cum_value);
    }
}
...
GCOV_LINKAGE void
gcov_read_histogram (struct gcov_histogram *histogram)
{
  unsigned h_ix, bv_ix, h_cnt = 0;
  ...
  while (h_cnt--)
      ...
      while (!cur_bitvector)
	{
	  ...
	  if (bv_ix >= GCOV_HISTOGRAM_BITVECTOR_SIZE)
	    gcov_error ("corrupted profile info: histogram "
	...
	gcov_error ("corrupted profile info: histogram "
      ...
      histogram->histogram[h_ix].num_counters = gcov_read_unsigned ();
      histogram->histogram[h_ix].min_value = gcov_read_counter ();
      histogram->histogram[h_ix].cum_value = gcov_read_counter ();
