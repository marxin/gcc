				    ...
				    bool case_sensitive)
{
  ...
  if (p1 && p2 && !case_sensitive)
    {
      int result;
      if (length != -1)
	result = strncmp (p1, p2, length);
      else
	result = strcmp (p1, p2);

      replace_call_with_value (gsi, build_cmp_result (type, result));
      return true;
    }
  ...
  if (p2 && *p2 == '\0' && length != 0)
    {
      ...
      tree temp = fold_build2_loc (loc, MEM_REF, cst_uchar_ptr_node, str1,
				   off0);
      temp = gimple_build (&stmts, loc, NOP_EXPR, cst_uchar_ptr_node, temp);
      gsi_insert_seq_before (gsi, stmts, GSI_SAME_STMT);
  ...
  if (p1 && *p1 == '\0' && length != 0)
    {
      ...
      tree temp = fold_build2_loc (loc, MEM_REF, cst_uchar_ptr_node, str2,
				   off0);
      temp = gimple_build (&stmts, loc, NOP_EXPR, cst_uchar_ptr_node, temp);
  ...
  if (!case_sensitive && length == 1)
    {
      gimple_seq stmts = NULL;

      tree temp1 = fold_build2_loc (loc, MEM_REF, cst_uchar_ptr_node, str1,
				    off0);
      temp1 = gimple_build (&stmts, loc, NOP_EXPR, cst_uchar_ptr_node, temp1);
      temp1 = gimple_convert (&stmts, loc, type, temp1);
      tree temp2 = fold_build2_loc (loc, MEM_REF, cst_uchar_ptr_node, str2,
				    off0);
      temp2 = gimple_build (&stmts, loc, NOP_EXPR, cst_uchar_ptr_node, temp2);
...
gimple_fold_builtin (gimple_stmt_iterator *gsi)
{
  ...
  switch (fcode)
    ...
    case BUILT_IN_STRCMP:
      return gimple_fold_builtin_string_compare (gsi, false);
    case BUILT_IN_STRCASECMP:
      return gimple_fold_builtin_string_compare (gsi, true);
