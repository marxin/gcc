...
hsa_output_kernel_mapping (tree brig_decl)
{
  ...
  finish_builtin_struct (kernel_info_type, "kernel_info", id_f4,
			 NULL_TREE);
  ...
  for (unsigned i = 0; i < map_count; ++i)
      ...
      if (hsa_decl_kernel_dependencies)
	{
      ...
      if (count > 0)
	{
	  ASM_GENERATE_INTERNAL_LABEL (tmp_name, "hsa_dependencies_list", i);
  ...
  ASM_GENERATE_INTERNAL_LABEL (tmp_name, "hsa_kernels", 1);
  ...
  append_to_statement_list
    (build_call_expr (reg_fn, 2,
		      build_fold_addr_expr (hsa_kernels),
		      build_fold_addr_expr (hsa_img_descriptor)),
     &hsa_ctor_statements);
