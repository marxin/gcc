...
func_checker::compare_ssa_name (tree t1, tree t2)
{
  ...
  if (m_comparing_rhs_assign && m_tail_merge_mode)
    return t1 == t2 ||
...
{
  m_comparing_rhs_assign = false;
}

/* Set flag that we compare RHS from a gimple assign.  */

void
func_checker::set_comparing_rhs_assign ()
{
  m_comparing_rhs_assign = true;
}
