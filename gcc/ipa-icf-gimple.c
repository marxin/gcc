...
func_checker::compare_ssa_name (tree t1, tree t2)
{
  ...
  if (!m_comparing_lhs_assign && m_tail_merge_mode)
    return t1 == t2 ||
...
{
  m_comparing_lhs_assign = false;
}

/* Set flag that we compare LHS from a gimple assign.  */

void
func_checker::set_comparing_lhs_assign ()
{
  m_comparing_lhs_assign = true;
}
  ...
  for (i = 0; i < gimple_num_ops (s1); i++)
    {
      ...
      /* Set indication that we're going to compare LHS of a gimple assign.  */
      if (i == 0)
	set_comparing_lhs_assign ();
