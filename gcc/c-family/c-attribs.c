...
/* Exclude FLAGS from options enabled by -fsanitize for function DECL.  */

void
add_no_sanitize_value (unsigned int flags, tree decl)
{
  struct gcc_options opts = global_options;

  cl_optimization_restore (&opts, opts_for_fn (decl));
  opts.x_flag_no_sanitize_fn |= flags;
			      ...
			      bool *no_add_attrs)
{
  ...
  add_no_sanitize_value (flags, *node);
				      ...
				      bool *no_add_attrs)
{
  if (TREE_CODE (*node) != FUNCTION_DECL)
    ...
    add_no_sanitize_value (SANITIZE_ADDRESS, *node);
				      ...
				      bool *no_add_attrs)
{
  if (TREE_CODE (*node) != FUNCTION_DECL)
    ...
    add_no_sanitize_value (SANITIZE_THREAD, *node);
