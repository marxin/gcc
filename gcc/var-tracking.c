...
typedef struct attrs_def
{
  ...
  {
    if (pool == NULL)
      pool = new pool_allocator <attrs_def> ("attrs_def pool", 1024);
...
typedef struct location_chain_def
{
  ...
  {
    if (pool == NULL)
      pool = new pool_allocator <location_chain_def>
...
typedef struct loc_exp_dep_s
{
  ...
  {
    if (pool == NULL)
      pool = new pool_allocator <loc_exp_dep_s> ("loc_exp_dep pool", 64);
...
typedef struct shared_hash_def
{
  ...
  {
    if (pool == NULL)
      pool = new pool_allocator <shared_hash_def> ("shared_hash_def pool", 256);
...
pool_allocator<attrs_def> *attrs_def::pool = NULL;

/* Alloc pool for struct variable_def with MAX_VAR_PARTS entries.  */

static pool_allocator<variable_def> *var_pool = NULL;

/* Alloc pool for struct variable_def with a single var_part entry.  */
static pool_allocator<variable_def> *valvar_pool = NULL;

/* Alloc pool for struct location_chain_def.  */
pool_allocator<location_chain_def> *location_chain_def::pool = NULL;

/* Alloc pool for struct shared_hash_def.  */
pool_allocator<shared_hash_def> *shared_hash_def::pool = NULL;

/* Alloc pool for struct loc_exp_dep_s for NOT_ONEPART variables.  */
pool_allocator<loc_exp_dep> *loc_exp_dep::pool = NULL;
...
vt_initialize (void)
{
  ...
  var_pool = new pool_allocator <variable_def>
    ("variable_def pool", 64,
     (MAX_VAR_PARTS - 1) * sizeof (((variable)NULL)->var_part[0]));

  valvar_pool = new pool_allocator <variable_def>
...
vt_finalize (void)
{
  ...
  attrs_def::release_pool ();
  var_pool->release ();
  location_chain_def::release_pool ();
  shared_hash_def::release_pool ();
