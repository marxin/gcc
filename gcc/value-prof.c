...
dump_histogram_value (FILE *dump_file, histogram_value hist)
{
  switch (hist->type)
    ...
    case HIST_TYPE_LOOP:
      if (hist->hvalue.counters)
	{
	  fprintf (dump_file, "Loop histogram: ");
	  if (hist->hvalue.counters)
	    {
	      for (unsigned i = 0; i < GCOV_LOOP_COUNTERS; i++)
		{
		  fprintf (dump_file, "[%d:%" PRId64 "]", i,
			   (int64_t) hist->hvalue.counters[i]);
				  ...
				  struct function *ofun, gimple *ostmt)
{
  ...
  for (val = gimple_histogram_value (ofun, ostmt); val != NULL; val = val->hvalue.next)
    {
      histogram_value new_val = gimple_alloc_histogram_value (fun, val->type, NULL, NULL, NULL);
...
static void
loop_counter_to_profile (basic_block bb, histogram_values *values)
{
...
gimple_find_values_to_profile (histogram_values *values)
{
  ...
  FOR_EACH_BB_FN (bb, cfun)
    {
      loop_counter_to_profile (bb, values);
      for (gsi = gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi))
	gimple_values_to_profile (gsi_stmt (gsi), values);
    }

  values->safe_push (gimple_alloc_histogram_value (cfun, HIST_TYPE_TIME_PROFILE, 0, 0, NULL));
