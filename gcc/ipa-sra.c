...
process_isra_node_results (cgraph_node *node,
			   ...
			   unsigned *removed_params,
			   unsigned *removed_returns,
			   unsigned *split_orig_params,
			   unsigned *split_new_params)
{
  isra_func_summary *ifs = func_sums->get (node);
  if (!ifs)
    return;
...
ipa_sra_analysis (void)
{
  ...
  unsigned removed_params = 0;
  unsigned removed_returns = 0;
  unsigned split_orig_params = 0;
  unsigned split_new_params = 0;

  cgraph_node *node;
  FOR_EACH_FUNCTION_WITH_GIMPLE_BODY (node)
    process_isra_node_results (node, clone_num_suffixes,
			       &removed_params, &removed_returns,
			       &split_orig_params, &split_new_params);

  if (removed_params || removed_returns
      || split_orig_params || split_new_params)
    {
      inform (UNKNOWN_LOCATION,
	      "%s:IPA-SRA removed %u params and %u return values and "
	      "split %u parameters into %u new ones.",
	      aux_base_name,
	      removed_params, removed_returns,
	      split_orig_params, split_new_params);
    }
