...
get_hsa_kernel_dispatch_offset (const char *field_name)
{
  ...
  for (tree chain = TYPE_FIELDS (hsa_kernel_dispatch_type);
    ...
    {
      if (strcmp (field_name, IDENTIFIER_POINTER (DECL_NAME (chain)))
	  == 0)
	{
	  tree byte_offset = DECL_FIELD_OFFSET (chain);
	  tree bit_offset = DECL_FIELD_BIT_OFFSET (chain);

	  return TREE_INT_CST_LOW (byte_offset)
	    + TREE_INT_CST_LOW (bit_offset) / BITS_PER_UNIT;
	}
    }
