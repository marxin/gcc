...
{
  hsa_op_immed *immed = (hsa_op_immed *) hsa_allocp_operand_immed->vallocate ();
  hsa_operands.safe_push (immed);
  return immed;
}
...
{
  hsa_op_reg *reg = (hsa_op_reg *) hsa_allocp_operand_reg->vallocate ();
  hsa_operands.safe_push (reg);
  return reg;
}
...
{
  hsa_op_address *addr = (hsa_op_address *)
    hsa_allocp_operand_address->vallocate ();
  hsa_operands.safe_push (addr);
  return addr;
}
...
{
  hsa_op_code_list *l = (hsa_op_code_list *)
    hsa_allocp_operand_code_list->vallocate ();
  hsa_operands.safe_push (l);
  return l;
}
...
{
  hsa_op_operand_list *l = (hsa_op_operand_list *)
    hsa_allocp_operand_operand_list->vallocate ();
  hsa_operands.safe_push (l);
  return l;
}
...
{
  hsa_insn_basic *i = (hsa_insn_basic *) hsa_allocp_inst_basic->vallocate ();
  hsa_instructions.safe_push (i);
  return i;
}
...
{
  hsa_insn_phi *phi = (hsa_insn_phi *) hsa_allocp_inst_phi->vallocate ();
  hsa_instructions.safe_push (phi);
  return phi;
}
...
{
  hsa_insn_br *br = (hsa_insn_br *) hsa_allocp_inst_br->vallocate ();
  hsa_instructions.safe_push (br);
  return br;
}
...
{
  hsa_insn_sbr *sbr = (hsa_insn_sbr *) hsa_allocp_inst_sbr->vallocate ();
  hsa_instructions.safe_push (sbr);
  return sbr;
}
...
{
  hsa_insn_sbr *cmp = (hsa_insn_sbr *) hsa_allocp_inst_cmp->vallocate ();
  hsa_instructions.safe_push (cmp);
  return cmp;
}
...
{
  hsa_insn_mem *mem = (hsa_insn_mem *) hsa_allocp_inst_mem->vallocate ();
  hsa_instructions.safe_push (mem);
  return mem;
}
...
{
  hsa_insn_atomic *atomic = (hsa_insn_atomic *)
    hsa_allocp_inst_atomic->vallocate ();
  hsa_instructions.safe_push (atomic);
  return atomic;
}
...
{
  hsa_insn_signal *signal = (hsa_insn_signal *)
    hsa_allocp_inst_signal->vallocate ();
  hsa_instructions.safe_push (signal);
  return signal;
}
...
{
  hsa_insn_seg *seg = (hsa_insn_seg *) hsa_allocp_inst_seg->vallocate ();
  hsa_instructions.safe_push (seg);
  return seg;
}
...
{
  hsa_insn_call *call = (hsa_insn_call *) hsa_allocp_inst_call->vallocate ();
  hsa_instructions.safe_push (call);
  return call;
}
...
{
  hsa_insn_arg_block *block = (hsa_insn_arg_block *)
    hsa_allocp_inst_arg_block->vallocate ();
  hsa_instructions.safe_push (block);
  return block;
}
...
{
  hsa_insn_comment *comment = (hsa_insn_comment *)
    hsa_allocp_inst_comment->vallocate ();
  hsa_instructions.safe_push (comment);
  return comment;
}
...
{
  hsa_insn_packed *packed = (hsa_insn_packed *)
    hsa_allocp_inst_packed->vallocate ();
  hsa_instructions.safe_push (packed);
  return packed;
}
				...
				hsa_bb *hbb, BrigType16_t reqtype)
{
  ...
  if (hsa_needs_cvt (reqtype, reg->m_type))
    {
			...
			vec <hsa_op_reg_p> *ssa_map)
{
  ...
  if (TREE_CODE (rhs) == SSA_NAME)
      ...
      else if (TREE_CODE (rhs) == COMPLEX_CST)
	{
	  ...
	  unsigned dst_type_length = hsa_type_bit_size (imag_part_reg->m_type);
	  BrigType16_t packed_type = BRIG_TYPE_B64;
	  BrigType16_t src_type = hsa_bittype_for_type (real_part_reg->m_type);

	  hsa_insn_packed *insn = new hsa_insn_packed
	    (3, BRIG_OPCODE_COMBINE, packed_type, src_type, dest, real_part_reg,
	     imag_part_reg);
	  hbb->append_insn (insn);
	}
      else
  ...
  if (spill_reg->m_type == BRIG_TYPE_B1)
    {
      ...
      *ptmp2 = reg;
      ...
      cvtinsn = new hsa_insn_basic
	(2, BRIG_OPCODE_CVT, reg->m_type, reg, *ptmp2);
	...
	if (hsa_needs_cvt (dest->m_type, v->m_type))
	  {
	    ...
	    hsa_insn_basic *cvtinsn = new hsa_insn_basic
	      (2, BRIG_OPCODE_CVT, dest->m_type, dest, tmp);
...
{
  if (formal_arg_type == NULL || AGGREGATE_TYPE_P (formal_arg_type))
    return actual_arg_type;
...
gen_num_threads_for_dispatch (hsa_bb *hbb)
{
  ...
  hbb->append_insn (new hsa_insn_basic (2, BRIG_OPCODE_CVT, dest->m_type,
					dest, tmp2));
				    ...
				    hsa_bb *hbb, vec <hsa_op_reg_p> *ssa_map)
{
  ...
  unsigned HOST_WIDE_INT mmodel = tree_to_uhwi (gimple_call_arg (stmt, 2));
			...
			vec <hsa_op_reg_p> *ssa_map)
{
  ...
  switch (DECL_FUNCTION_CODE (fndecl))
	...
	hsa_op_address *addr;
	addr = get_address_from_value (gimple_call_arg (stmt, 0), hbb, ssa_map);
	unsigned HOST_WIDE_INT mmodel = tree_to_uhwi
