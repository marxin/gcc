			       ...
			       vec <hsa_op_reg_p> *ssa_map)
{
  ...
  tree parm_decl_chain = TYPE_ARG_TYPES (gimple_call_fntype (stmt));
  ...
  for (unsigned i = 0; i < args; ++i)
    {
      ...
      tree parm_decl_type = parm_decl_chain != NULL_TREE
	? TREE_VALUE (parm_decl_chain) : NULL_TREE;
      hsa_op_address *addr;
      ...
      else
	{
      ...
      if (parm_decl_chain)
	parm_decl_chain = TREE_CHAIN (parm_decl_chain);
    }
  ...
  if (!VOID_TYPE_P (result_type))
      ...
      if (result)
	{
	  ...
	  BrigType16_t mtype = mem_type_for_type
	    (hsa_type_for_tree_type (lhs_type, &dim));
			...
			vec <hsa_op_reg_p> *ssa_map)
{
  ...
  if (retval)
      ...
      else
	{
	  unsigned HOST_WIDE_INT dim;
	  BrigType16_t mtype = mem_type_for_type
	    (hsa_type_for_tree_type (TREE_TYPE (retval), &dim));
			     ...
			     vec <hsa_op_reg_p> *ssa_map)
{
  ...
  for (parm = DECL_ARGUMENTS (cfun->decl), i = 0;
	...
	return;
	 ...
	 for them.  */
      hsa_symbol *private_arg = hsa_cfun->create_hsa_temporary (arg->m_type);
