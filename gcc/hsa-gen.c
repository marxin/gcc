...
/* Set output operand of the instruction to DEST, as the DEST
   can have a different type, conversion instructions are possibly
   appended to HBB.  */

void
hsa_insn_basic::set_output_in_type (hsa_op_reg *dest, hsa_bb *hbb)
{
  hsa_insn_basic *insn;
  unsigned i = get_op_output_index ();
...
/* Generate a special HSA-related instruction to a DEST register.
		...
		hsa_bb *hbb)
{
  ...
  hsa_insn_basic *insn = new hsa_insn_basic (2, opcode, BRIG_TYPE_U32, NULL,
					     imm);
  hbb->append_insn (insn);
  insn->set_output_in_type (dest, hbb);
}
	...
	for (unsigned i = 1; i < labels; i++)
	  {
	    ...
	    if (!types_compatible_p (TREE_TYPE (low), index_type))
		...
		if (!types_compatible_p (TREE_TYPE (high), index_type))
