...
#define HSA_WARNING_MSG "could not emit HSAIL for the function"

#define HSA_WARN_ATV(location, message, ...) \
  do \
  { \
...
#define HSA_WARN_AT(location, message) \
  do \
  { \
    hsa_cfun->seen_error = true; \
    if (warning_at (EXPR_LOCATION (hsa_cfun->decl), OPT_Whsa, \
...
hsa_type_for_scalar_tree_type (const_tree type, bool min32int)
{
  ...
  if (!tree_fits_uhwi_p (TYPE_SIZE (base)))
    {
      HSA_WARN_ATV (EXPR_LOCATION (type),
		    "support for HSA does not implement huge or variable-sized "
		    "type %T", type);
      return res;
    }
			...
			bool min32int = false)
{
  ...
  if (!tree_fits_uhwi_p (TYPE_SIZE_UNIT (type)))
    {
      HSA_WARN_ATV (EXPR_LOCATION (type), "support for HSA does not "
  ...
  if (TREE_CODE (tree_value) == STRING_CST)
	   ...
	   i++)
	{
	  ...
	  if (!CONSTANT_CLASS_P (v))
	    {
	      HSA_WARN_AT (EXPR_LOCATION (tree_val),
		  ...
		  vec <hsa_op_reg_p> *ssa_map)
{
  if (TREE_CODE (base) == SSA_NAME)
	...
	{
	  HSA_WARN_AT (EXPR_LOCATION (base),
	      ...
	      HOST_WIDE_INT *output_bitpos = NULL)
{
  ...
  if (TREE_CODE (ref) == STRING_CST)
    ...
    {
      HSA_WARN_ATV (EXPR_LOCATION (origref),
  ...
  switch (TREE_CODE (ref))
      ...
      if (TMR_INDEX2 (ref))
	{
      ...
      break;
    case FUNCTION_DECL:
      HSA_WARN_AT (EXPR_LOCATION (origref),
		   "support for HSA does not implement function pointers");
      goto out;
    case SSA_NAME:
    default:
      HSA_WARN_ATV (EXPR_LOCATION (origref), "support for HSA does "
...
out:
  ...
  if ((bitpos || bitsize) && (output_bitpos == NULL || output_bitsize == NULL))
    HSA_WARN_ATV (EXPR_LOCATION (origref), "support for HSA does not "
  ...
  if (TREE_CODE (rhs) == SSA_NAME)
      ...
      if (POINTER_TYPE_P (TREE_TYPE (rhs)))
	{
	  if (dest->type != hsa_get_segment_addr_type (BRIG_SEGMENT_FLAT))
	    {
	      HSA_WARN_ATV (EXPR_LOCATION (rhs),
			      ...
			      vec <hsa_op_reg_p> *ssa_map)
{
  ...
  switch (code)
    ...
    default:
      HSA_WARN_ATV (EXPR_LOCATION (lhs),
					...
					vec <hsa_op_reg_p> *ssa_map)
{
  ...
  switch (code)
    ...
    case FLOOR_DIV_EXPR:
    case ROUND_DIV_EXPR:
      HSA_WARN_AT (gimple_location (assign),
		   "support for HSA does not "
		   "implement CEIL_DIV_EXPR, FLOOR_DIV_EXPR or ROUND_DIV_EXPR");
      return;
    case TRUNC_MOD_EXPR:
    ...
    case ROUND_MOD_EXPR:
      HSA_WARN_AT (gimple_location (assign),
		   "support for HSA does not "
		   "implement CEIL_MOD_EXPR, FLOOR_MOD_EXPR or ROUND_MOD_EXPR");
      return;
    case NEGATE_EXPR:
    ...
    default:
      ...
      HSA_WARN_ATV (gimple_location (assign),
...
verify_function_arguments (tree decl)
{
  if (DECL_STATIC_CHAIN (decl))
    {
      HSA_WARN_ATV (EXPR_LOCATION (decl),
			       ...
			       vec <hsa_op_reg_p> *ssa_map)
{
  ...
  for (unsigned i = 0; i < args; ++i)
	...
	{
	  HSA_WARN_AT (gimple_location (stmt),
		       "support for HSA does not "
  ...
  if (!VOID_TYPE_P (result_type))
	...
	{
	  HSA_WARN_ATV (gimple_location (stmt),
...
get_address_from_value (tree val, hsa_bb *hbb, vec <hsa_op_reg_p> *ssa_map)
{
  switch (TREE_CODE (val))
    ...
    default:
      HSA_WARN_ATV (EXPR_LOCATION (val),
  ...
  for (unsigned i = 0; i < count; i++)
      ...
      else
	{
	  ...
	  if (!POINTER_TYPE_P (t)
	    ...
	    {
	      HSA_WARN_AT (gimple_location (phi_stmt),
