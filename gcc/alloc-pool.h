...
private:
  ...
  /* These are the elements that have been allocated at least once and freed.  */
  alloc_pool_list m_returned_free_list;
  ...
  m_call_ctor (!ignore_type_size)
{
  ...
  if (GATHER_STATISTICS)
    {
      struct alloc_pool_descriptor *desc = allocate_pool_descriptor (m_name);
      desc->elt_size = size;
      desc->created++;
    }
...
pool_allocator<T>::allocate ()
{
...
#ifdef ENABLE_VALGRIND_ANNOTATIONS
  size = elt_size - offsetof (allocation_object, u.data);
#endif
  ...
  ALLOCATION_OBJECT_PTR_FROM_USER_PTR (header)->id = m_id;
#endif
...
pool_allocator<T>::remove (T *object)
{
  ...
  gcc_assert (object
	      ...
	      && m_id == ALLOCATION_OBJECT_PTR_FROM_USER_PTR (object)->id);

  memset (object, 0xaf, size);

  /* Mark the element to be free.  */
  ALLOCATION_OBJECT_PTR_FROM_USER_PTR (object)->id = 0;
#endif

  header = (alloc_pool_list) object;
