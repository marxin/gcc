  ...
  if (cgraph_node *cnode = dyn_cast <cgraph_node *> (node))
    {
      ...
      if (!node->alias)
	part->insns += ipa_fn_summaries->get_create (cnode)->self_size;
...
undo_partition (ltrans_partition partition, unsigned int n_nodes)
{
  while (lto_symtab_encoder_size (partition->encoder) > (int)n_nodes)
	...
	partition->insns -= ipa_fn_summaries->get_create (cnode)->self_size;
...
lto_balanced_map (int n_lto_partitions, int max_partition_size)
{
  ...
  unsigned int best_n_nodes = 0, best_i = 0, best_cost = INT_MAX,
	       best_internal = 0;
  int npartitions;
  int current_order = -1;
  int noreorder_pos = 0;
  ...
  for (i = 0; i < order.length (); i++)
	     ...
	     && noreorder[noreorder_pos]->order < current_order)
	{
	  if (!noreorder[noreorder_pos]->alias)
	    total_size
	      -= ipa_fn_summaries->get_create (noreorder[noreorder_pos])->size;
      ...
      if (!order[i]->alias)
	total_size -= ipa_fn_summaries->get_create (order[i])->size;
