  ...
  if (cgraph_node *cnode = dyn_cast <cgraph_node *> (node))
    {
      ...
      if (!node->alias)
	part->insns += inline_summaries->get_or_insert (cnode)->self_size;
...
undo_partition (ltrans_partition partition, unsigned int n_nodes)
{
  while (lto_symtab_encoder_size (partition->encoder) > (int)n_nodes)
	...
	partition->insns -= inline_summaries->get_or_insert (cnode)->self_size;
...
lto_balanced_map (int n_lto_partitions, int max_partition_size)
{
  ...
  unsigned int best_n_nodes = 0, best_i = 0, best_cost =
  ...
  FOR_EACH_DEFINED_FUNCTION (node)
	...
	if (!node->alias)
	  total_size += inline_summaries->get_or_insert (node)->size;
      }
  ...
  for (i = 0; i < order.length (); i++)
	     ...
	     && noreorder[noreorder_pos]->order < current_order)
	{
	  if (!noreorder[noreorder_pos]->alias)
	    {
	      inline_summary *s;
	      s = inline_summaries->get_or_insert (noreorder[noreorder_pos]);
	      total_size -= s->size;
	    }
      ...
      if (!order[i]->alias)
	total_size -= inline_summaries->get_or_insert (order[i])->size;
