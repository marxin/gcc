			    ...
			    unsigned int low, unsigned int high)
{
  ...
  intervals.qsort (cmp_tuples);

  for (unsigned i = 0; i < intervals.length () - 1; i++)
    {
  ...
  for (i = 0; i < debug_counter_number_of_counters; i++)
    {
      ...
      if (limits[i] != NULL)
	{
	  auto_vec<limit_tuple> &intervals = *limits[i];
	  for (int j = intervals.length () - 1; j >= 0; j--)
	    {
	      printf ("[%u, %u]", intervals[j].first, intervals[j].second);
	      if (j > 0)
		printf (", ");
	    }
	  putchar ('\n');
