...
get_indirect_branch_type_from_string (const char *value)
{
  ...
  if (strcmp (value, "keep") == 0)
    ...
    return indirect_branch_thunk;
  else if (strcmp (value, "thunk-inline") == 0)
    return indirect_branch_thunk_inline;
  else if (strcmp (value, "thunk-extern") == 0)
    return indirect_branch_thunk_extern;
  else
    return indirect_branch_unset;
}
...
#define MAX_CODE_ALIGN 16
  ...
  if (ix86_indirect_branch)
  {
    enum indirect_branch value
      = get_indirect_branch_type_from_string (ix86_indirect_branch);
    if (value == indirect_branch_unset)
      error ("bad value (%s) for -mindirect-branch= switch",
	     ix86_indirect_branch);
  }

  if (ix86_function_return)
  {
    enum indirect_branch value
      = get_indirect_branch_type_from_string (ix86_function_return);
    if (value == indirect_branch_unset)
      error ("bad value (%s) for -mfunction-return= switch",
...
/* Output a call and return thunk for indirect branch.
   If REGNO != -1,  the function
...
output_indirect_thunk (int regno)
{
  ...
  if (regno >= 0)
  ...
  else
    {
      ...
      output_asm_insn ("lea\t{%a1, %0|%0, %a1}", xops);
    }
...
ix86_set_indirect_branch_type (tree fndecl)
{
  if (cfun->machine->indirect_branch_type == indirect_branch_unset)
      ...
      if (attr != NULL)
	{
	  ...
	  ix86_set_indirect_branch_type_from_string (TREE_STRING_POINTER (cst));
	}
      else
...
ix86_output_indirect_branch_via_reg (rtx call_op, bool sibcall_p)
{
  ...
  char *thunk_name;
  /* A sibcall via r11 register.  */
  int regno = call_op == NULL_RTX ? R11_REG : REGNO (call_op);
...
static void
ix86_output_indirect_branch_via_push (rtx call_op, bool sibcall_p)
{
  char thunk_name_buf[32];
  char *thunk_name;
  char push_buf[64];
...
static void
ix86_output_indirect_branch (rtx call_op, bool sibcall_p)
{
  if (REG_P (call_op))
    ix86_output_indirect_branch_via_reg (call_op, sibcall_p);
  else
    ix86_output_indirect_branch_via_push (call_op, sibcall_p);
}
...
ix86_output_indirect_jmp (rtx call_op, bool ret_p)
{
  if (cfun->machine->indirect_branch_type != indirect_branch_keep)
	...
	gcc_unreachable ();
      ...
      ix86_output_indirect_branch (call_op, true);
      return "";
    }
  else
    return "jmp\t%A0";
...
{
  bool r11_sibcall_p = call_op == NULL_RTX;
  bool direct_p = !r11_sibcall_p && constant_call_address_operand (call_op, Pmode);
  ...
  if (SIBLING_CALL_P (insn))
      ...
      else
	{
	  if (output_indirect_p)
	    {
	      if (r11_sibcall_p)
		ix86_output_indirect_branch_via_reg (NULL_RTX, true);
	      else
		ix86_output_indirect_branch (call_op, true);
	      return "";
	    }
  ...
  if (direct_p)
  ...
  else
    {
      if (output_indirect_p)
	{
	  ix86_output_indirect_branch (call_op, false);
	  return "";
	}
      else
