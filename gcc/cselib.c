...
struct elt_list
{
  ...
  {
    if (pool == NULL)
      pool = new pool_allocator <elt_list> ("elt_list", 10);
...
pool_allocator<elt_list> *elt_list::pool = NULL;
pool_allocator<elt_loc_list> *elt_loc_list::pool = NULL;
pool_allocator<cselib_val> *cselib_val::pool = NULL;

static pool_allocator<rtx_def> *value_pool = NULL;
...
{
  value_pool = new pool_allocator <rtx_def> ("value", 100,
					     RTX_CODE_SIZE (VALUE),
					     true);
...
cselib_finish (void)
{
  ...
  elt_list::release_pool ();
  elt_loc_list::release_pool ();
  cselib_val::release_pool ();
  value_pool->release ();
  cselib_clear_table ();
