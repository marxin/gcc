...
sem_function::equals (sem_item *item)
{
  if (dump_file && (dump_flags & TDF_DETAILS))
    fprintf (dump_file, "\nEquals called for:%s:%s (%s:%s)\n",
	     name(), item->name (), asm_name (), item->asm_name ());
	   ...
	   gsi_next (&gsi))
	{
	  ...
	  switch (code)
		...
		if (gimple_num_ops (stmt) == 2
		  ...
		  {
		    tree addr_expr = gimple_op (stmt, 1);
		    tree addr_expr_arg = TREE_OPERAND (addr_expr, 0);

		    if (TREE_CODE (addr_expr_arg) == FUNCTION_DECL)
		      tree_refs.safe_push (addr_expr_arg);
		  }
...
sem_function::check_operand (tree t1, tree t2, tree func1, tree func2)
{
  ...
  if ((handled_component_p (t1) && handled_component_p (t2))
      || tc1 == ADDR_EXPR || tc1 == MEM_REF || tc1 == REALPART_EXPR
      || tc1 == IMAGPART_EXPR || tc1 == OBJ_TYPE_REF)
    {
...
congruence_class::dump (FILE *file, unsigned int indent) const
{
  ...
  for (unsigned i = 0; i < members.length (); i++)
    fprintf (file, "%s ", members[i]->asm_name ());
