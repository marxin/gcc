...
class Profile:
    ...
    def branches_max(self):
        return max([v.branches for k, v in self.heuristics.iteritems()])

    def count_max(self):
        return max([v.count for k, v in self.heuristics.iteritems()])

    def dump(self):
        print('%-36s %8s %6s  %-16s %14s %8s %6s' % ('HEURISTICS', 'BRANCHES', '(REL)',
              'HITRATE', 'COVERAGE', 'COVERAGE', '(REL)'))
        for (k, v) in sorted(self.heuristics.iteritems(), key = lambda x: x[1].branches):
