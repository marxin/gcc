...
hsa_warn (const char *str, hsa_status_t status)
{
  ...
  fprintf (stderr, "HSA fatal error: %s (%s)\n", str, err);
...
create_and_finalize_hsa_program (struct agent_info *agent)
{
  ...
  if (status != HSA_STATUS_SUCCESS)
    {
      hsa_warn ("Could not create an HSA program", status);
      goto failure;
    }
  ...
  while (module)
    {
      ...
      if (status != HSA_STATUS_SUCCESS)
	{
	  hsa_warn ("Could not add a module to the HSA program", status);
	  goto failure;
	}
      module = module->next;
      mi++;
  ...
  if (status != HSA_STATUS_SUCCESS)
    {
      hsa_warn ("Could not create HSA executable", status);
      goto failure;
    }
  ...
  if (status != HSA_STATUS_SUCCESS)
    {
      hsa_warn ("Could not add a code object to the HSA executable", status);
      goto failure;
    }
  status = hsa_executable_freeze(agent->executable, "");
  if (status != HSA_STATUS_SUCCESS)
    {
      hsa_warn ("Could not freeze the HSA executable", status);
      goto failure;
    }
...
init_single_kernel (struct kernel_info *kernel, unsigned *max_omp_data_size)
{
  ...
  if (status != HSA_STATUS_SUCCESS)
    {
      hsa_warn ("Could not extract a kernel object from its symbol", status);
      goto failure;
    }
  status = hsa_executable_symbol_get_info
  ...
  if (status != HSA_STATUS_SUCCESS)
    {
      hsa_warn ("Could not get info about kernel argument size", status);
      goto failure;
    }
  status = hsa_executable_symbol_get_info
  ...
  if (status != HSA_STATUS_SUCCESS)
    {
      hsa_warn ("Could not get info about kernel group segment size", status);
      goto failure;
    }
  status = hsa_executable_symbol_get_info
