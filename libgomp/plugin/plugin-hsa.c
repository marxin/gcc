...
hsa_warn (const char *str, hsa_status_t status)
{
  ...
  hsa_fns.hsa_status_string_fn (status, &hsa_error);
...
{
  void *handle = dlopen (HSA_RUNTIME_LIB "libhsa-runtime64.so", RTLD_LAZY);
  if (handle == NULL)
    return false;
...
init_hsa_context (void)
{
  ...
  if (!init_hsa_runtime_functions ())
    GOMP_PLUGIN_fatal ("Run-time could not be dynamically opened");
