...
/* Trim last character of the STRING, caller of the function is responsible
   for releasing the returned memory.  */

static char *
trim_last_char (const char *string)
{
  unsigned len = strlen (string);
  char *copy = GOMP_PLUGIN_malloc (len);
...
hsa_warn (const char *str, hsa_status_t status)
{
  ...
  char *hsa_error_trimmed = trim_last_char (hsa_error);

  fprintf (stderr, "HSA warning: %s (%s)\n", str, hsa_error_trimmed);
  free (hsa_error_trimmed);
}
...
hsa_fatal (const char *str, hsa_status_t status)
{
  ...
  hsa_status_string (status, &hsa_error);
  char *hsa_error_trimmed = trim_last_char (hsa_error);
  GOMP_PLUGIN_fatal ("HSA fatal error: %s (%s)", str, hsa_error_trimmed);
  free (hsa_error_trimmed);
}
